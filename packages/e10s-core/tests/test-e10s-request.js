// This is just a copy of test-request.js.
...
exports.testStatus_200 = function (test) {
  ...
  Request({
    url: "http://playground.zpao.com/jetpack/request/text.php",
    onComplete: function () {
      test.assertEqual(this.response.status, 200);
      test.assertEqual(this.response.statusText, "OK");
      test.done();
    }
  }).get();
}
...
exports.testStatus_404 = function (test) {
  ...
  Request({
    ...
    onComplete: function () {
      test.assertEqual(this.response.status, 404);
      test.assertEqual(this.response.statusText, "Not Found");
      test.done();
    }
  }).get();
}
...
exports.testSimpleXML = function (test) {
  test.pass("TODO: Skipping XML testing for now.");
  /*
  test.waitUntilDone();
  Request({
...
exports.testSimpleText = function (test) {
  ...
  Request({
    url: "http://playground.zpao.com/jetpack/request/text.php",
    onComplete: function () {
      test.assertEqual(this.response.text, "Look ma, no hands!\n");
      test.done();
    }
  }).get();
}
...
exports.testKnownHeader = function (test) {
  ...
  Request({
    ...
    onComplete: function () {
      test.assertEqual(this.response.headers["x-zpao-header"], "Jamba Juice");
      test.done();
    }
  }).get();
}
...
exports.testKnownHeader = function (test) {
  ...
  Request({
    ...
    onComplete: function () {
      for (k in headers) {
        test.assertEqual(this.response.headers[k], headers[k]);
      }
      test.done();
    }
  }).get();
...
exports.testContentTypeHeader = function (test) {
  ...
  Request({
    ...
    onComplete: function () {
      test.assertEqual(this.response.headers["Content-Type"], "text/plain");
      test.done();
    }
  }).get();
}
...
exports.testSimpleJSON = function (test) {
  ...
  Request({
    ...
    onComplete: function () {
      assertDeepEqual(test, this.response.json, { foo: "bar" });
      test.done();
    }
  }).get();
}
...
exports.testInvalidJSON = function (test) {
  ...
  Request({
    ...
    onComplete: function () {
      test.assertEqual(this.response.json, null);
      test.done();
    }
  }).get();
}
...
exports.testGetWithParamsNotContent = function (test) {
  ...
  Request({
    ...
    onComplete: function () {
      ...
      assertDeepEqual(test, this.response.json, expected);
      test.done();
    }
  }).get();
}
